...
def test_csv_builders():
    ...
    lcsv = logs_csv([{"id": 1, "action": "a", "details": "d", "created_at": "2026-01-01"}])
    acsv = analytics_csv({"users_total": 10})
    assert "users_total,10" in acsv
    assert "id,action,details,created_at" in lcsv
    assert "id,username,user_type,establishment,registered_at" in ucsv
